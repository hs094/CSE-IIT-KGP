                    ...
                    | INTEGER_CONSTANT
                        {
                            ...
                            $$ = new Expression();
                            $$->symbol = gentemp(SymbolType::int_, toString($1));
                            emit("=", $$->symbol->name, $1);
                        }
                    | FLOATING_CONSTANT
                        {
                            ...
                            $$ = new Expression();
                            $$->symbol = gentemp(SymbolType::float_, $1);
                            emit("=", $$->symbol->name, $1);
                        }
                    | CHARACTER_CONSTANT
                        {
                            ...
                            $$ = new Expression();
                            $$->symbol = gentemp(SymbolType::char_, $1);
                            emit("=", $$->symbol->name, $1);
                        }
                    | STRING_LITERAL
                        {
                            ...
                            $$ = new Expression();
                   $$->symbol = gentemp(SymbolType::pointer_, $1);
                   $$->symbol->type->arrayType = new SymbolType(SymbolType::char_);
                        }
                    | '(' expression ')'
                        {
                    ...
                    | postfix_expression '[' expression ']'
                        {
                            ...
                            $$->temp = gentemp(SymbolType::int_); // temporary to compute location
                            ...
                            if($1->type == Array::ARRAY) {
                                ...
                                Symbol *sym = gentemp(SymbolType::int_);
                    ...
                    | unary_operator cast_expression
                        {
                            ...
                            if(strcmp($1, "&") == 0) {
                                ...
                                $$ = new Array();
                                $$->symbol = gentemp(SymbolType::pointer_);
                    ...
                    | shift_expression LEFT_SHIFT additive_expression
                        {
                            ...
                            if($3->symbol->type->type == SymbolType::int_) {
                                $$ = new Expression();
                                $$->symbol = gentemp(SymbolType::int_);
                                emit("<<", $$->symbol->name, $1->symbol->name, $3->symbol->name);
                            } else {
                                yyerror("Type error.");
                            }
                ...
                VOIDTYPE
                    {
                        ...
                        currentType = SymbolType::void_;
                    }
                | CHARTYPE
                    {
                        yyinfo("type_specifier => char");
                        currentType = SymbolType::char_;
                    }
                | SHORT
                    {
                    ...
                    | direct_declarator '[' assignment_expression ']'
                        {
                            ...
                            while(it1->type == SymbolType::array_) { // go to the base level of a nested type
                                it2 = it1;
                                it1 = it1->arrayType;
                            }
                            if(it2 != NULL) { // nested array case
                                // another level of nesting with base as it1 and width the value of assignment_expression
                                it2->arrayType =  new SymbolType(SymbolType::array_, it1, atoi($3->symbol->initialValue.c_str()));
                                $$ = $1->update($1->type);
                            }
                            else { // fresh array
                                // create a new array with base as type of direct_declarator and width the value of assignment_expression
                                $$ = $1->update(new SymbolType(SymbolType::array_, $1->type, atoi($3->symbol->initialValue.c_str())));
                            }
                        }
                    | direct_declarator '[' ']'
                        {
                            ...
                            if(it2 != NULL) { // nested array case
                                // another level of nesting with base as it1 and width the value of assignment_expression
                                it2->arrayType =  new SymbolType(SymbolType::array_, it1, 0);
                                $$ = $1->update($1->type);
                            }
                            else { // fresh array
                                // create a new array with base as type of direct_declarator and width the value of assignment_expression
                                $$ = $1->update(new SymbolType(SymbolType::array_, $1->type, 0));
                            }
                        }
                    ...
                    | direct_declarator '(' change_scope parameter_type_list ')'
                        {
                            ...
                            if($1->type->type != SymbolType::void_) {
        ...
        '*' type_qualifier_list_opt
            {
                ...
                // fresh pointer
                $$ = new SymbolType(SymbolType::pointer_);
            }
        | '*' type_qualifier_list_opt pointer
            {
                yyinfo("pointer => * type_qualifier_list_opt pointer");
                // nested pointer
                $$ = new SymbolType(SymbolType::pointer_, $3);
            }
        ;
...
change_block:
                    {
                        ...
                        s->type = new SymbolType(SymbolType::block_);
                        currentSymbol = s;
                    }
                ;
