...
map<char, int> escapeCharactersAsciiValues = {{'n', 10}, {'t', 9}, {'r', 13}, {'b', 8}, {'f', 12}, {'v', 11}, {'a', 7}, {'0', 0}};
int getAsciiValue(string charConst) {
    if(charConst.length() == 3)
        return (int)charConst[1];
    else // escaped character
        return escapeCharactersAsciiValues.find(charConst[2]) != escapeCharactersAsciiValues.end() ? escapeCharactersAsciiValues[charConst[2]] : (int)charConst[2];
}
...
string getStackLoc(string paramName) {
    return (currentAR->displacement.count(paramName)) ? toString(currentAR->displacement[paramName]) + "(%rbp)" : paramName;
}
void loadParam(string paramName, int paramNum) {     // register to stack
    Symbol *symbol = currentTable->lookup(paramName);
    int size = symbol->size;
    SymbolType::typeEnum type = symbol->type->type;
    string movIns = "";
    if(type == SymbolType::ARRAY) {     // if it is an array just store the address
        movIns = "movq";
        size = 8;
    } 
    else { switch(size) {
            case 1: movIns = "movb"; break;
            case 4: movIns = "movl"; break;
            case 8: movIns = "movq"; break;
        }
    }
...
void translate() {
    ...
    stack<string> params; // stack to store the params for function calls, especially helpful for nested function call of the type fun1(fun2(arg1, arg2), fun3(arg3, arg4))
    quadNum = 1;
    for(auto &quad:quadArray) {
        if(quad->op == "label") {
            if(!inTextSpace) {
            ...
            if(inTextSpace) {
                ...
                if(op == "=") {
                    ...
                    if(isdigit(arg1[0]))   // integer constant
                        assemblyFile << "\t" << setw(8) << "movl" << "$" << arg1 << ", " << getStackLoc(result) << endl;
                    else if(arg1[0] == '\'')  // character constant
                ...
                else if(op == "goto")   // unconditional jump
                    ...
                    switch(sz)
                    {
                        case 1:             movins = "movb";    cmpins = "cmpb";    movreg = "%al";    break;
                        case 4:             movins = "movl";    cmpins = "cmpl";    movreg = "%eax";   break;
                        case 8:             movins = "movq";    cmpins = "cmpq";    movreg = "%rax";   break;
                    }
                ...
                if(currentSymbol->category == Symbol::TEMPORARY) {
                    switch(currentSymbol->type->type)
                    {
                        case SymbolType::INT:   globalIntTemp = stoi(quad->arg1); break;
                        case SymbolType::CHAR:  globalCharTemp = getAsciiValue(quad->arg1); break; 
                        case SymbolType::POINTER:  globalStringTemp = ".LC" + quad->arg1; break;
                    }
                } 
                else {
...
int main(int argc, char const *argv[]) {
    ...
    yyparse();
    globalTable->update();   // update the symbol table and generate activation records
    globalTable->print();   // print the symbol tables and quads
    finalBackpatch();
    int ins = 1;
    for(auto it : quadArray) cout<< setw(4) << ins++ << ": ", it->print();
    translate();    // generate the assembly file
    return 0;
}